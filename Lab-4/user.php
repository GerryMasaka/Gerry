...
class User implements Account 
{
    ...
    */ 
    ...
    public function logout ($pdo)
    {
     ...
     public function order ($pdo)
    {
        
        try 
        {
            $stmt = $pdo->prepare ('INSERT INTO orders (email,food,units) VALUES(?,?,$_SESSION["email"])');
            $stmt->execute([$this->getfood(),$this->getunits()]);
             echo '<script>alert("Order made")</script>';               
                echo '<script>window.location.href="IAPLogOut.php"</script>';                
